...
/*   Updated: 2023/02/23 10:29:27 by pcheron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_tab	*ft_make_tab2(t_tab *tab, int argc, char **argv, char **env)
{
	...
	if (!tab->fd)
		return (ft_free_strs(tab->files, 2), ft_free_strs(tab->env_path, i), free(tab), NULL);
	tab->pipes = ft_build_pipes(tab->nb_pipes);
	if (!tab->pipes)
		return (free(tab->fd), ft_free_strs(tab->files, 2), ft_free_strs(tab->env_path, i), free(tab), NULL);
	tab->cmd = ft_build_cmd_tab(argc, argv);
	if (!tab->cmd)
		return (ft_free_pipes(2, tab->pipes), free(tab->fd), ft_free_strs(tab->files, 2), ft_free_strs(tab->env_path, i), free(tab), NULL);
	return (tab);
}
